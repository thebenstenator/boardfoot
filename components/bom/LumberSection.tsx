...
function inferPricingMode(species: string): LumberItem['pricing_mode'] {
  ...
  if (!s) return 'per_bf';
  if (/^\d+x\d+/.test(s)) return 'per_piece';
...
export function LumberSection({ projectId }: LumberSectionProps) {
  const { items, addItem, updateItem, removeItem, undoRemove, reorderItems } = useLumberItems(projectId);
  ...
  function handleUpdate(id: string, field: keyof LumberItem, raw: string) {
    const numericFields = ["thickness_in", "width_in", "length_ft", "quantity", "price_per_unit", "waste_override"];
  ...
  function handleUnitToggle(item: LumberItem) {
    if (item.length_unit === "ft") {
      updateItem(item.id, { length_unit: "in", length_ft: parseFloat((item.length_ft * 12).toFixed(3)) });
    } else {
      updateItem(item.id, { length_unit: "ft", length_ft: parseFloat((item.length_ft / 12).toFixed(3)) });
    }
  }

  function getLineBF(item: LumberItem): number {
    const bf = calculateBoardFeetFlexible(item.thickness_in, item.width_in, item.length_ft, (item.length_unit ?? "ft") as LengthUnit);
    return bf * item.quantity;
  }

  function getLineTotal(item: LumberItem): number {
    if (item.pricing_mode === "per_piece") return item.price_per_unit * item.quantity;
    const bf = getLineBF(item);
    if (item.pricing_mode === "per_lf") {
      return bf * convertLFtoBFPrice(item.price_per_unit, item.thickness_in, item.width_in);
    }
    ...
    <div className={bomSection}>
      <div className={bomSectionHeader}>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowPanelCalc(true)} className="cursor-pointer">
            Panel calc
          </Button>
          <Button
            size="sm"
                      ...
                      return (
                        <Fragment key={item.id}>
                          ...
                          <SortableRow id={item.id}>
                            <div data-lumber-row className={`${bomRow} group border-b hover:bg-muted/30`}>
                              <div className={`${col.first} relative`} title={item.species}>
                                <SpeciesInput
                                  ...
                                  onChange={(species, suggestedPrice, dimensions) => {
                                    ...
                                    if (suggestedPrice !== undefined) updateItem(item.id, { price_per_unit: suggestedPrice });
                                    if (dimensions) updateItem(item.id, { thickness_in: dimensions.thickness, width_in: dimensions.width });
                                  }}
                                  tabIndex={baseTab}
                                />
                                <button
                                  onClick={() => updateItem(item.id, { is_reclaimed: !item.is_reclaimed })}
                                  tabIndex={-1}
                                  aria-label={item.is_reclaimed ? "Unmark reclaimed" : "Mark as reclaimed/on-hand"}
                                  title={item.is_reclaimed ? "Reclaimed/on-hand — click to unmark" : "Mark as reclaimed"}
                                  className={`absolute -top-0.5 -right-0.5 text-xs rounded px-0.5 leading-none
                                    ...
                                    ${item.is_reclaimed ? "opacity-100 text-green-600" : "opacity-0 group-hover:opacity-60 hover:!opacity-100 text-muted-foreground"}`}
                                >
                                  ♻
                                </button>
                              </div>
                              <div className={`${col.sm} text-right`}>
                                <EditableCell value={item.thickness_in} onChange={(v) => handleUpdate(item.id, "thickness_in", v)} type="number" tabIndex={baseTab + 1} />
                                {nominalLabel && (
                                  <span className="block text-[10px] text-muted-foreground leading-none mt-0.5 text-center">{nominalLabel}</span>
                                )}
                              </div>
                              <div className={`${col.sm} text-right`}>
                                <EditableCell value={item.width_in} onChange={(v) => handleUpdate(item.id, "width_in", v)} type="number" tabIndex={baseTab + 2} />
                              </div>
                              <div className={`${col.md} pl-1`}>
                                <input
                                  ...
                                  className="w-full bg-transparent border border-transparent rounded px-1 py-0.5
                                    text-sm focus:outline-none focus:border-ring focus:bg-background hover:border-border"
                                />
                              </div>
                              ...
                              <div className={col.sm}>
                                <EditableCell value={item.quantity} onChange={(v) => handleUpdate(item.id, "quantity", v)} type="number" tabIndex={baseTab + 5} />
                              </div>
                              <div className={col.pricingMode}>
                                ...
                                >
                                  <SelectTrigger className="h-7 text-xs border-transparent hover:border-border focus:border-ring px-1.5" aria-label={`Pricing mode: ${item.pricing_mode}`}>
                                    <SelectValue />
                                  </SelectTrigger>
                                  <SelectContent>
                              ...
                              <div className={`${col.lg} relative`}>
                                <CurrencyCell
                                  ...
                                  onChange={(v) => {
                                    ...
                                    setStalePriceIds((prev) => { const next = new Set(prev); next.delete(item.id); return next; });
                                  }}
                                  tabIndex={baseTab + 7}
                                />
                                {stalePriceIds.has(item.id) && (
                                  <span title="Unit changed — update price to match" className="absolute -top-0.5 -right-0.5 text-[10px] text-amber-500 leading-none pointer-events-none">⚠</span>
                                )}
                              </div>
                              <div className={`${col.sm} text-right text-muted-foreground text-sm`}>
                                {item.pricing_mode === "per_piece" ? "—" : totalBF.toFixed(2)}
                              </div>
                              <div className={`${col.last} text-sm`}>
                                {item.is_reclaimed ? <span className="text-green-600">$0.00</span> : formatCurrency(lineTotal)}
                              </div>
